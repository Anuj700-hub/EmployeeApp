...
public class MenuOptionListAdapter extends RecyclerView.Adapter<MenuOptionViewHolder> {
    ...
    List<SubProduct> options;
    LayoutInflater inflater;
    MainApplication mainApplication;
    ArrayList<OrderSubProduct> orderOptions;
    ...
    public MenuOptionListAdapter(AppCompatActivity activity, List<SubProduct> options,
                                 ArrayList<OrderSubProduct> orderSubProducts, boolean free, OrderProduct orderProduct,
                                 UpdateRefreshListener updateRefreshListener, Product product) {
    ...
    public void onBindViewHolder(final MenuOptionViewHolder holder, int position) {
        final SubProduct option = options.get(position);
...
//        }
        if (tempOrderSubproduct == null) {
            tempOrderSubproduct = new OrderSubProduct();
    ...
    private void showOptionItemDialog(final MenuOptionViewHolder holder, final SubProduct option, final OrderSubProduct orderSubProduct) {
        holder.llOptionItemContainer.removeAllViews();
        for (final OptionItem optionItem : option.getMenuOptionsItems()) {
...
//            }
            ...
            final OptionItem finalOptionItem = optionItem;
    ...
    private CheckBox getFirstNonSelected(LinearLayout container, OptionItem optionItemToIgnore) {
        for (int i = 0; i < container.getChildCount(); i++) {
            ...
            if (view instanceof LinearLayout) {
                if (((LinearLayout) view).getChildCount() == 3 && ((LinearLayout) view).getChildAt(2) instanceof CheckBox) {
                    ...
                    OptionItem checkBoxOptionItem = (OptionItem) cbSelect.getTag();
    ...
    private CheckBox getFirstSelected(LinearLayout container, OptionItem optionItemToIgnore) {
        for (int i = 0; i < container.getChildCount(); i++) {
            View view = container.getChildAt(i);
            if (view instanceof LinearLayout) {
                if (((LinearLayout) view).getChildCount() == 3 && ((LinearLayout) view).getChildAt(2) instanceof CheckBox) {
    ...
    private void onOptionItemChecked(final MenuOptionViewHolder holder, final SubProduct subProduct,
                                     final OrderSubProduct orderSubProduct, boolean checked, final OptionItem optionItem) {
        ...
        if (checked) {
            if (orderSubProduct.getOrderOptionItems().size() >= subProduct.getMaximumSelection()) {
                ...
                if(checkBox!=null) {
                    final OptionItem checkBoxOptionItem = (OptionItem) checkBox.getTag();

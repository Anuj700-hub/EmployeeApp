...
public class ITLDeviceCom extends Thread implements DeviceSetupListener, DeviceEventListener, DeviceFileUpdateListener {
    ...
    public void run(){
        ...
        while(isrunning){
            ...
            if(cfg.configUpdate == SSPComsConfig.ComsConfigChangeState.ccNewConfig){
                ...
                CashRecharge.getInstance().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        CashRecharge.getInstance().SetConfig(cfg.baud,cfg.dataBits,cfg.stopBits,cfg.parity,cfg.flowControl);
                    }
                });
    ...
    public void OnNewDeviceSetup(final SSPDevice dev) {
        ...
        CashRecharge.getInstance().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                CashRecharge.getInstance().DisplaySetUp(dev);
            }
        });
    ...
    public void OnDeviceDisconnect(final SSPDevice dev) {
        ...
        CashRecharge.getInstance().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                CashRecharge.getInstance().DeviceDisconnected(dev);
            }
        });
    }
    ...
    public void OnDeviceEvent(final DeviceEvent ev)
    {
        CashRecharge.getInstance().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                CashRecharge.getInstance().DisplayEvents(ev);
            }
        });
    }
    ...
    public void OnFileUpdateStatus(final SSPUpdate sspUpdate) {
        ...
        CashRecharge.getInstance().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                CashRecharge.getInstance().UpdateFileDownload(sspUpdate);
            }
        });
